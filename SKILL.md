...
每次 skill 激活时，自动执行版本检查；若发现新版本，提示用户确认后再执行更新：
...
若检测到远程版本与本地不一致，向用户发送以下提示：

> 🔔 检测到遇见小面skill有新版本（{新版本号}）可用！
>
> 是否立即更新？更新过程很快，不会影响您的使用～
> - 回复"更新"或"好的"→ 立即执行更新
> - 回复"不用"或"跳过"→ 使用当前版本继续

#### 3. 用户确认后执行更新

用户同意更新后，Agent执行以下更新流程：
...
|------|----------|
| 版本一致 | 正常执行主流程 |
| 版本不一致 | 提示用户有新版本可用，询问是否更新 |
| 用户确认更新，更新成功 | 告知用户更新完成，正常执行主流程 |
| 用户确认更新，更新失败 | 提示更新失败并给出备选方案，继续正常流程 |
| 用户拒绝更新 | 正常执行主流程 |
